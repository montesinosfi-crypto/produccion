...
def page(title, body, user=None):
    ...
    if user:
        ...
        links_edit = (
            ...
            '<a href="/agenda">Agenda</a><a href="/estadisticas">Estadisticas</a><a href="/productividad">Productividad</a><a href="/productividad">Productividad</a>'
        )
        links_ro = (
            '<a href="/">Inicio</a><a href="/programas">Programas</a>'
...
@login_required
def productividad():
    """Resumen de desempeno personal: tendido (metros, horas), ordenes, piezas de corte asociadas."""
    periodo = (request.args.get("periodo") or "semana").strip()  # dia | semana | mes
    ...
    conn = get_db(); cur = dcur(conn)
    # Tendidos en rango (por fecha_inicio)
    try:
        cur.execute(
            "SELECT t.*, o.numero_orden, o.piezas_cortadas, o.fecha_final_corte, o.fecha_tampografia, o.tampografia_lista "
    ...
    # Ordenes con corte finalizado en el periodo (para total de piezas del area)
    try:
        cur.execute(
            "SELECT * FROM ordenes WHERE fecha_final_corte IS NOT NULL AND fecha_final_corte >= " + ph() +
            " AND fecha_final_corte <= " + ph(),
            (fi_s, ff_s)
        )
    ...
    # Tampografia en periodo
    try:
        cur.execute(
            "SELECT * FROM ordenes WHERE fecha_tampografia IS NOT NULL AND fecha_tampografia >= " + ph() +
            " AND fecha_tampografia <= " + ph(),
            (fi_s, ff_s)
        )
    ...
    # Agregar por persona
    personas = {}  # nombre -> metrics

    def ensure(p):
        if p not in personas:
            personas[p] = {
                "tendidos": 0, "bloques": 0, "metros": 0.0, "mins": 0,
                "ordenes_tendido": set(), "piezas_corte": 0, "ordenes_corte": set(),
                "tampografias": 0, "detalle": []
            }
        return personas[p]
    ...
    for t in tendidos:
        ...
        for p in pair:
            ...
            # Si la orden ya tiene piezas cortadas, se atribuyen a quienes tendieron (dividido entre la pareja)
            if pzas > 0 and oid:
                share = pzas / max(len(pair), 1)
                if oid not in st["ordenes_corte"]:
                    st["piezas_corte"] += share
                    st["ordenes_corte"].add(oid)
            st["detalle"].append({
                "dia": dia, "orden": num, "mins": mins or 0, "metros": metros,
                "bloques": n_bl, "piezas": (pzas / max(len(pair), 1)) if pzas else 0
            })

    # Totales area corte/tampografia (no siempre atribuibles a persona)
    ...
    total_mins_rep = sum(st["mins"] for st in personas.values())

    # Ordenar personas por metros
    ...
    for nombre, st in lista:
        ...
        rows += (
            ...
            len(st["ordenes_tendido"]), st["piezas_corte"], pct
        )
    if not rows:
        rows = '<tr><td colspan="8">Sin registros de tendido en este periodo. Registra pareja e inicio/fin en las ordenes.</td></tr>'
    ...
    if persona_q and persona_q in personas:
        ...
        for d in sorted(st["detalle"], key=lambda x: x["dia"], reverse=True):
            drows += "<tr><td>%s</td><td>%s</td><td>%s</td><td>%.1f m</td><td>%s min</td><td>%.0f</td></tr>" % (
                d["dia"], d["orden"], d["bloques"], d["metros"], d["mins"], d["piezas"]
            )
        detalle_html = (
            ...
            'Horas reportadas: <strong>%.1f h</strong> · Ordenes: <strong>%s</strong> · '
            'Piezas corte (asociadas): <strong>%.0f</strong></p>'
            '<table><thead><tr><th>Fecha</th><th>Orden</th><th>Bloques</th><th>Metros</th><th>Minutos</th><th>Piezas*</th></tr></thead>'
            '<tbody>%s</tbody></table>'
            '<p style="font-size:.8rem;color:#64748b;margin-top:.5rem">* Piezas de corte de la orden, repartidas entre la pareja de tendido.</p>'
    ...
    body = (
        ...
        '<p style="color:#64748b">Area tendido / corte / tampografia · Horario: Lun 8:00-18:00 (9 h netas) · '
        'Mar-Vie 8:00-17:30 (8.5 h netas) · Comida 14:00-15:00</p>'
        ...
        '<div class="stat"><div class="n">%.1f h</div><div class="l">Horas reportadas (tendido)</div></div>'
        '<div class="stat"><div class="n">%s</div><div class="l">Ordenes cortadas (fecha fin)</div></div>'
        '<div class="stat"><div class="n">%s</div><div class="l">Piezas cortadas</div></div>'
        '<div class="stat"><div class="n">%s</div><div class="l">Tampografias (fecha)</div></div>'
        '</div></div>'
        % (total_metros, total_mins_rep/60.0, total_ordenes_corte, total_piezas_corte, total_tampos)
        +
        '<div class="card"><h2>Desempeno por persona</h2>'
        '<p style="font-size:.85rem;color:#64748b;margin-bottom:.5rem">Metros y horas salen del tendido (pareja). '
        'Piezas de corte se reparten entre quienes tendieron esa orden. %% = horas reportadas / horas laborables del periodo.</p>'
        '<table><thead><tr>'
        '<th>Persona</th><th>Tendidos</th><th>Bloques</th><th>Metros</th><th>Horas rep.</th>'
        '<th>Ordenes</th><th>Piezas*</th><th>%% jornada</th>'
        '</tr></thead><tbody>' + rows + '</tbody></table></div>'
        + detalle_html
        +
        '<div class="card"><h2>Como se calcula</h2>'
        '<ul style="margin-left:1.2rem;font-size:.88rem;color:#475569;line-height:1.5">'
        '<li><strong>Metros</strong> = largo del bloque × cantidad de lienzos (suma de bloques del tendido).</li>'
        '<li><strong>Horas reportadas</strong> = tiempo entre inicio y fin de cada tendido completo.</li>'
        '<li><strong>Horas laborables</strong> = Lun 9 h · Mar-Vie 8.5 h (ya descontada comida).</li>'
        '<li><strong>Piezas</strong> = piezas cortadas de la orden, divididas entre la pareja de tendido.</li>'
        '<li>Si alguien solo corta y no esta en tendido, no aparece aqui: registra tendido o pide campo de operador de corte.</li>'
        '</ul></div>'
    )
