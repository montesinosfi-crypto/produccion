...
app = Flask(__name__)
APP_VERSION = "2026-09-24-v7b-fix"
...
def init_db():
    ...
    if c == 0:
        ...
        for u in [
            ...
            ("ADUANA", h("ADUANA123"), "Aduana", "Aduana"),
        ]:
            if USE_POSTGRES:
    ...
    # Asegurar usuarios y contraseñas estandar (conn sigue abierta)
    try:
        usuarios_std = [
            ...
            if USE_POSTGRES:
                ...
                if row:
                    ...
                    cur.execute("INSERT INTO usuarios (usuario,password_hash,nombre,area,activo) VALUES (?,?,?,?,1)",
                                (u, hpw, nom, area))
        conn.commit()
        print("usuarios OK: admin, programa, trazo, corte, gerente, habilitado, DISEÑO, ADUANA")
    except Exception as e:
    ...
    try:
        if USE_POSTGRES:
            ...
            for col in ["muestra_maestra","aduana_entrega_ok","aduana_fecha_entrega","aduana_pasos_ok","aduana_usuario","aduana_obs"]:
                try: cur.execute("ALTER TABLE ordenes ADD COLUMN "+col+" TEXT")
                except Exception: pass
        conn.commit()
        print("aduana cols ok")
    except Exception as e:
        print("aduana cols:", e)
...
def is_admin_or_gerente():
    ...
    return (
        area in ("admin", "administrador", "gerente")
        or usuario in ("admin", "gerente")
    )

def orden_cerrada_por_aduana(o):
    """True si aduana confirmo entrega completa (orden bloqueada)."""
    if not o:
        return False
    try:
        if str(o.get("orden_cerrada") or "").lower() in ("1", "si", "sí", "yes", "true"):
            return True
        ent = str(o.get("aduana_entrega_ok") or "").lower()
        pasos = str(o.get("aduana_pasos_ok") or "").lower()
        if ent in ("si", "sí", "yes") and pasos in ("si", "sí", "yes"):
            return True
    except Exception:
        return False
    return False
...
def factor_tipo_orden(tipo):
    """Pedido especial x2, programa x1."""
    t = (tipo or "").strip().lower()
    if "especial" in t or t == "especial":
        return 2.0
    return 1.0

def factor_modelo(modelo):
    """Pijama x2, bata x1.5, pantalon/filipina x1, gorro/bolsa x0.5."""
    m = (modelo or "").strip().lower()
    if not m:
        return 1.0
    if "pijama" in m or "pyjama" in m:
        return 2.0
    if "bata" in m:
        return 1.5
    if "gorro" in m or "bolsa" in m:
        return 0.5
    if "pantalon" in m or "pantalón" in m or "filipina" in m:
        return 1.0
    return 1.0

def piezas_productividad(piezas, modelo, tipo):
    """Piezas equivalentes = piezas_cortadas x factor_modelo x factor_tipo."""
    try:
        p = float(piezas or 0)
    except Exception:
        p = 0.0
    return p * factor_modelo(modelo) * factor_tipo_orden(tipo)
...
def page(title, body, user=None):
    ...
    if user:
        ...
        links_edit = (
            ...
            '<a href="/agenda">Agenda</a><a href="/estadisticas">Estadisticas</a><a href="/productividad">Productividad</a><a href="/checklist">Checklist</a>'
        )
        links_ro = (
            '<a href="/">Inicio</a><a href="/programas">Programas</a>'
    ...
    return (
        ...
        + nav + '<div class="container"><div style="text-align:right;font-size:.65rem;color:#94a3b8;margin-bottom:.3rem">v ' + APP_VERSION + '</div>' + body + "</div>"
...
def login():
    ...
    if request.method == "POST":
        ...
        try:
            conn = get_db(); cur = dcur(conn)
            # buscar usuario sin importar mayusculas
            if USE_POSTGRES:
                cur.execute("SELECT * FROM usuarios WHERE LOWER(usuario)=LOWER(%s) AND activo=1", (u,))
            else:
                cur.execute("SELECT * FROM usuarios WHERE LOWER(usuario)=LOWER(?) AND activo=1", (u,))
...
def ver_orden(orden_id):
    ...
    if request.method == "POST":
        if not can_edit():
            return page("Solo lectura",
                '<div class="alert alert-err">Tu usuario solo puede consultar. No tiene permisos para modificar.</div>'
                f'<a href="/orden/{orden_id}" class="btn btn-outline">Volver</a>', session)
        if orden_cerrada_por_aduana(o) and not is_admin_or_gerente():
            return page("Orden cerrada",
                '<div class="alert alert-err">Esta orden fue confirmada por Aduana. Solo Gerente o Admin pueden modificarla.</div>'
                f'<a href="/orden/{orden_id}" class="btn btn-outline">Volver</a>', session)
    ...
    cerrada = orden_cerrada_por_aduana(o)
    banner_cierre = ""
    if cerrada:
        banner_cierre = (
            '<div class="alert alert-ok" style="border-left:4px solid #059669">'
            '<strong>✓ Envio a maquila COMPLETADO</strong> — Confirmado por Aduana'
        )
        if o.get("aduana_fecha_entrega"):
            banner_cierre += " el <strong>" + str(o.get("aduana_fecha_entrega")) + "</strong>"
        if o.get("aduana_usuario"):
            banner_cierre += " · " + str(o.get("aduana_usuario"))
        if not is_admin_or_gerente():
            banner_cierre += '<br><span style="font-size:.85rem">Orden cerrada: no se puede editar (solo Gerente o Admin).</span>'
        else:
            banner_cierre += '<br><span style="font-size:.85rem">Puedes editar porque eres Admin/Gerente.</span>'
        banner_cierre += "</div>"

    body = f'''{banner_cierre}<div class="card">
    ...
    if cerrada and not is_admin_or_gerente():
        body = body.replace('<form method="POST"', '<div class="locked-form" style="opacity:.9;pointer-events:none"')
        body = body.replace('</form>', '<p style="font-size:.75rem;color:#94a3b8;margin-top:.3rem">Edicion bloqueada (orden cerrada por Aduana)</p></div>')
        body = body.replace('type="submit"', 'type="button" disabled')
...
def productividad():
    """
    TENDIDO: solo metros + horas (eficiencia), SIN piezas.
    CORTE: piezas equivalentes = piezas_cortadas x factor_modelo x factor_tipo.
    """
    ...
    # --- TENDIDOS (metros + tiempo) ---
    try:
        cur.execute(
            "SELECT t.*, o.numero_orden, o.modelo, o.color, o.tipo "
    ...
    # --- CORTES (piezas con formula) ---
    try:
        cur.execute(
            "SELECT * FROM ordenes WHERE fecha_final_corte IS NOT NULL AND fecha_final_corte != '' "
            "AND fecha_final_corte >= " + ph() + " AND fecha_final_corte <= " + ph() +
            " ORDER BY fecha_final_corte, numero_orden",
            (fi_s, ff_s)
        )
    ...
    # ===== TENDIDO por persona: metros + horas (NO piezas) =====
    pers_tend = {}

    def et(p):
        if p not in pers_tend:
            pers_tend[p] = {
    ...
    for t in tendidos:
        ...
        for p in pair:
            ...
            if oid:
                st["ordenes"].add(oid)
            st["detalle"].append({
                "dia": dia, "orden": num, "mins": mins or 0,
                "metros": metros, "bloques": n_bl,
                "inicio": str(t.get("hora_inicio") or ""),
                "fin": str(t.get("hora_fin") or ""),
            })

    total_metros = sum(st["metros"] for st in pers_tend.values())
    lista_tend = sorted(pers_tend.items(), key=lambda x: x[1]["metros"], reverse=True)

    # ===== CORTE: piezas equivalentes con formula =====
    # Piezas se atribuyen a quienes tendieron esa orden (total equipo, no dividido)
    # Si no hay tendido, solo sale en informe de area
    pers_corte = {}
    # map orden_id -> personas del tendido
    orden_personas = {}
    for t in tendidos:
        oid = t.get("orden_id")
        if not oid:
            continue
        for p in [(t.get("persona1") or "").strip(), (t.get("persona2") or "").strip()]:
            if p:
                orden_personas.setdefault(oid, set()).add(p)
    ...
    for c in cortes:
        oid = c.get("id")
        pzas = int(c.get("piezas_cortadas") or 0)
        modelo = c.get("modelo") or ""
        tipo = c.get("tipo") or "Programa"
        f_mod = factor_modelo(modelo)
        f_tip = factor_tipo_orden(tipo)
        pzas_eq = piezas_productividad(pzas, modelo, tipo)
        total_pzas_raw += pzas
        total_pzas_eq += pzas_eq
        informe_corte.append({
            "fecha": (c.get("fecha_final_corte") or "")[:10],
            "orden": c.get("numero_orden") or "",
            "modelo": modelo,
        ...
        personas = orden_personas.get(oid) or set()
        for p in personas:
            if p not in pers_corte:
                pers_corte[p] = {"ordenes": 0, "pzas_raw": 0, "pzas_eq": 0.0, "detalle": []}
            # una sola vez por orden
            ya = any(d.get("oid") == oid for d in pers_corte[p]["detalle"])
            if not ya:
                pers_corte[p]["ordenes"] += 1
                pers_corte[p]["pzas_raw"] += pzas
                pers_corte[p]["pzas_eq"] += pzas_eq
                pers_corte[p]["detalle"].append({
                    "oid": oid,
                    "fecha": (c.get("fecha_final_corte") or "")[:10],
                    "orden": c.get("numero_orden") or "",
                    "modelo": modelo,
    ...
    lista_corte = sorted(pers_corte.items(), key=lambda x: x[1]["pzas_eq"], reverse=True)

    # Piezas por dia (raw y equiv)
    piezas_dia = {}
    for c in cortes:
        d = (c.get("fecha_final_corte") or "")[:10]
        if not d:
            continue
        piezas_dia.setdefault(d, {"ordenes": 0, "pzas": 0, "pzas_eq": 0.0})
        piezas_dia[d]["ordenes"] += 1
        pr = int(c.get("piezas_cortadas") or 0)
        piezas_dia[d]["pzas"] += pr
        piezas_dia[d]["pzas_eq"] += piezas_productividad(pr, c.get("modelo"), c.get("tipo"))
    ...
    # Tabla tendido
    rows_t = ""
    for nombre, st in lista_tend:
        horas_rep = st["mins"] / 60.0
        pct = (horas_rep / horas_disp * 100) if horas_disp > 0 else 0
        m_h = (st["metros"] / horas_rep) if horas_rep > 0 else 0
        rows_t += (
            "<tr>"
            '<td><strong><a href="/productividad?periodo=%s&fecha=%s&persona=%s#tendido">%s</a></strong></td>'
            "<td>%s</td><td>%s</td><td><strong>%.1f m</strong></td>"
            "<td>%.1f h</td><td>%.0f%%</td><td>%.1f m/h</td>"
            "</tr>"
        ) % (
            periodo, fecha_ref, nombre.replace(" ", "%20"), nombre,
            st["tendidos"], st["bloques"], st["metros"],
            horas_rep, min(pct, 999), m_h
        )
    if not rows_t:
        rows_t = '<tr><td colspan="7">Sin registros de tendido en este periodo</td></tr>'

    # Tabla corte personas
    rows_c = ""
    for nombre, st in lista_corte:
        rows_c += (
            "<tr>"
            '<td><strong><a href="/productividad?periodo=%s&fecha=%s&persona=%s#corte">%s</a></strong></td>'
            "<td>%s</td><td>%s</td><td><strong>%.0f</strong></td>"
            "</tr>"
        ) % (
            periodo, fecha_ref, nombre.replace(" ", "%20"), nombre,
            st["ordenes"], st["pzas_raw"], st["pzas_eq"]
        )
    if not rows_c:
        rows_c = '<tr><td colspan="4">Sin cortes asociados a personal de tendido en el periodo</td></tr>'

    # Detalle persona
    detalle_html = ""
    if persona_q:
        det_t = ""
        if persona_q in pers_tend:
            st = pers_tend[persona_q]
            drows = ""
            for d in sorted(st["detalle"], key=lambda x: x["dia"], reverse=True):
                drows += "<tr><td>%s</td><td>%s</td><td>%s</td><td>%.1f m</td><td>%s min</td></tr>" % (
                    d["dia"], d["orden"], d["bloques"], d["metros"], d["mins"]
                )
            det_t = (
                '<div class="card" id="tendido"><h2>Tendido — ' + persona_q + '</h2>'
                '<p>Metros: <strong>%.1f m</strong> · Horas: <strong>%.1f h</strong> · Tendidos: %s</p>'
                '<table><thead><tr><th>Fecha</th><th>Orden</th><th>Bloques</th><th>Metros</th><th>Minutos lab.</th></tr></thead>'
                '<tbody>%s</tbody></table></div>'
            ) % (st["metros"], st["mins"]/60.0, st["tendidos"], drows or "<tr><td colspan=5>—</td></tr>")
        det_c = ""
        if persona_q in pers_corte:
            st = pers_corte[persona_q]
            drows = ""
            for d in st["detalle"]:
                drows += (
                    "<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td>"
                    "<td>%s</td><td>%.1f × %.1f</td><td><strong>%.0f</strong></td></tr>"
                ) % (
                    d["fecha"], d["orden"], d["modelo"], d["tipo"],
                    d["pzas"], d["f_mod"], d["f_tip"], d["pzas_eq"]
                )
            det_c = (
                '<div class="card" id="corte"><h2>Corte (piezas equivalentes) — ' + persona_q + '</h2>'
                '<p>Formula: <code>piezas × factor_modelo × factor_tipo</code></p>'
                '<p>Piezas reales: <strong>%s</strong> · Equivalentes: <strong>%.0f</strong></p>'
                '<table><thead><tr><th>Fecha</th><th>Orden</th><th>Modelo</th><th>Tipo</th>'
                '<th>Piezas</th><th>Factores</th><th>Equiv.</th></tr></thead>'
                '<tbody>%s</tbody></table></div>'
            ) % (st["pzas_raw"], st["pzas_eq"], drows)
        detalle_html = det_t + det_c
        if detalle_html:
            detalle_html += (
                '<p><a href="/productividad?periodo=%s&fecha=%s" class="btn btn-outline btn-sm">Cerrar detalle</a></p>'
                % (periodo, fecha_ref)
            )

    # Informe corte ordenes
    ord_rows = ""
    for c in informe_corte:
        ord_rows += (
            "<tr><td>%s</td><td><strong>%s</strong></td><td>%s</td><td>%s</td><td>%s</td>"
            "<td>%s</td><td>%.1f × %.1f</td><td><strong>%.0f</strong></td>"
            '<td><a href="/orden/%s" class="btn btn-outline btn-sm">Ver</a></td></tr>'
        ) % (
            c["fecha"], c["orden"], c["modelo"], c["color"], c["tipo"],
            c["pzas"], c["f_mod"], c["f_tip"], c["pzas_eq"], c["id"]
        )
    if not ord_rows:
        ord_rows = "<tr><td colspan=9>Sin ordenes cortadas en el periodo</td></tr>"

    pd_rows = "".join(
        "<tr><td>%s</td><td>%s</td><td>%s</td><td><strong>%.0f</strong></td></tr>" % (
            d, v["ordenes"], v["pzas"], v["pzas_eq"]
    ...
    body = (
        '<div class="card">'
        '<h1>Productividad</h1>'
        '<p style="color:#64748b;margin-bottom:.6rem">Tendido = metros y tiempo. Corte = piezas con formula de equivalencia.</p>'
        '<div style="display:flex;flex-wrap:wrap;gap:.4rem;align-items:center;margin-bottom:.6rem">' + tabs +
        '<form method="GET" style="display:inline-flex;gap:.4rem;align-items:center;margin-left:.4rem">'
        ...
        '<p><strong>' + titulo_per + '</strong> · Jornada disponible: <strong>%.1f h</strong></p></div>'
        % horas_disp
        +
        # KPIs
        ...
        '<div style="font-size:1.5rem;font-weight:700;color:#0f2744">%.1f m</div>'
        '<div style="font-size:.75rem;color:#64748b;text-transform:uppercase">Metros tendidos</div></div>'
        '<div class="card" style="margin:0;text-align:center;padding:1rem">'
        '<div style="font-size:1.5rem;font-weight:700;color:#0f2744">%.1f h</div>'
        '<div style="font-size:.75rem;color:#64748b;text-transform:uppercase">Horas tendido</div></div>'
        '<div class="card" style="margin:0;text-align:center;padding:1rem">'
        '<div style="font-size:1.5rem;font-weight:700;color:#0f2744">%s</div>'
        '<div style="font-size:.75rem;color:#64748b;text-transform:uppercase">Ordenes cortadas</div></div>'
        '<div class="card" style="margin:0;text-align:center;padding:1rem">'
        '<div style="font-size:1.5rem;font-weight:700;color:#0f2744">%.0f</div>'
        '<div style="font-size:.75rem;color:#64748b;text-transform:uppercase">Piezas equiv. corte</div></div>'
        '</div>'
        % (total_metros, total_mins_tend/60.0, len(cortes), total_pzas_eq)
        +
        # TENDIDO section
        '<div class="card">'
        '<h2>1. Tendido — metros y tiempo</h2>'
        '<p style="font-size:.82rem;color:#64748b;margin-bottom:.6rem">'
        'Sin piezas. Eficiencia = metros / hora laboral. Horas solo dentro de jornada (sin noche ni comida).</p>'
        '<table><thead><tr>'
        '<th>Persona</th><th>Tendidos</th><th>Bloques</th><th>Metros</th>'
        '<th>Horas</th><th>% jornada</th><th>m/h</th>'
        '</tr></thead><tbody>' + rows_t + '</tbody></table></div>'
        +
        # CORTE section
        '<div class="card">'
        '<h2>2. Corte — piezas equivalentes</h2>'
        '<p style="font-size:.82rem;color:#64748b;margin-bottom:.4rem">'
        '<strong>Formula:</strong> piezas_cortadas × factor_modelo × factor_tipo</p>'
        '<p style="font-size:.78rem;color:#64748b;margin-bottom:.6rem">'
        'Tipo: Especial ×2 · Programa ×1 &nbsp;|&nbsp; '
        'Modelo: Pijama ×2 · Bata ×1.5 · Pantalon/Filipina ×1 · Gorro/Bolsa ×0.5<br>'
        'Ejemplo: especial + pijama + 50 pzas = 50 × 2 × 2 = <strong>200</strong></p>'
        '<table><thead><tr>'
        '<th>Persona</th><th>Ordenes</th><th>Piezas reales</th><th>Piezas equivalentes</th>'
        '</tr></thead><tbody>' + rows_c + '</tbody></table>'
        '<p style="font-size:.75rem;color:#94a3b8;margin-top:.5rem">'
        'Se asigna el total del equipo a cada persona del tendido de esa orden (no se divide).</p></div>'
        +
        detalle_html
        +
        '<div class="card">'
        '<h2>Piezas de corte por dia</h2>'
        '<table><thead><tr><th>Fecha</th><th>Ordenes</th><th>Piezas reales</th><th>Equivalentes</th></tr></thead>'
        '<tbody>' + pd_rows + '</tbody></table></div>'
        +
        '<div class="card">'
        '<h2>Informe de ordenes cortadas</h2>'
        '<table style="font-size:.8rem"><thead><tr>'
        '<th>Fecha</th><th>Orden</th><th>Modelo</th><th>Color</th><th>Tipo</th>'
        '<th>Piezas</th><th>Factores</th><th>Equiv.</th><th></th>'
        '</tr></thead><tbody>' + ord_rows + '</tbody></table></div>'
    )
...
def checklist_confirmar(orden_id):
    ...
    # Si ya esta cerrada, solo admin/gerente pueden volver a tocar
    conn0 = get_db(); cur0 = dcur(conn0)
    cur0.execute("SELECT * FROM ordenes WHERE id=" + ph(), (orden_id,))
    o0 = cur0.fetchone(); conn0.close()
    if orden_cerrada_por_aduana(o0) and not is_admin_or_gerente():
        return page("Orden cerrada",
            '<div class="alert alert-err">Esta orden ya fue confirmada y cerrada. Solo Gerente o Admin pueden modificarla.</div>'
            '<a href="/checklist" class="btn btn-outline">Volver al checklist</a>', session)
    user = session.get("nombre") or session.get("usuario")
    ...
    try:
        if USE_POSTGRES:
            ...
            if str(entrega_ok).lower() in ("si", "sí", "yes") and str(pasos_ok).lower() in ("si", "sí", "yes"):
                if not fecha_ent:
                    fecha_ent = datetime.now().strftime("%Y-%m-%d")
                cur.execute("""UPDATE ordenes SET ubicacion=%s, estado_actual=%s,
                    fecha_salida_maquila=CASE WHEN fecha_salida_maquila IS NULL OR fecha_salida_maquila='' THEN %s ELSE fecha_salida_maquila END,
                    aduana_fecha_entrega=%s WHERE id=%s""",
                    ("Maquila", "Enviado_Maquila", fecha_ent, fecha_ent, orden_id))
        else:
            for col in ["muestra_maestra", "aduana_entrega_ok", "aduana_fecha_entrega", "aduana_pasos_ok", "aduana_usuario", "aduana_obs"]:
                try:
            ...
            if str(entrega_ok).lower() in ("si", "sí", "yes") and str(pasos_ok).lower() in ("si", "sí", "yes"):
                if not fecha_ent:
                    fecha_ent = datetime.now().strftime("%Y-%m-%d")
                cur.execute("SELECT fecha_salida_maquila FROM ordenes WHERE id=?", (orden_id,))
                row = cur.fetchone()
                fs = (row[0] if row and not isinstance(row, dict) else (row.get("fecha_salida_maquila") if row else None))
                if not fs:
                    cur.execute("UPDATE ordenes SET fecha_salida_maquila=? WHERE id=?", (fecha_ent, orden_id))
                cur.execute("UPDATE ordenes SET ubicacion=?, estado_actual=?, aduana_fecha_entrega=? WHERE id=?",
                            ("Maquila", "Enviado_Maquila", fecha_ent, orden_id))
        conn.commit()
        try:
        ...
        try:
            ...
            conn2.close()
            msg_n = "Aduana confirmo checklist"
            if str(entrega_ok).lower() in ("si", "sí", "yes") and str(pasos_ok).lower() in ("si", "sí", "yes"):
                msg_n = "Aduana confirmo entrega completa → ENVIADO a maquila"
            crear_notificacion(orden_id, num, msg_n)
        except Exception as e:
...
def checklist():
    ...
    ordenes = []
    try:
        cur.execute("SELECT * FROM ordenes ORDER BY id DESC LIMIT 800")
        ordenes = cur.fetchall() or []
        if fechas_set:
            ordenes = [o for o in ordenes if (str(o.get("fecha_salida_maquila") or "")[:10] in fechas_set)]
    except Exception as e:
    ...
    for o in ordenes:
        ...
        # Formulario aduana
        form_adu = ""
        cerrada_ord = orden_cerrada_por_aduana(o)
        if cerrada_ord and not is_admin_or_gerente():
            form_adu = (
                '<div style="margin-top:.35rem;padding:.4rem;background:#ecfdf5;border-radius:8px;font-size:.75rem;color:#065f46">'
                '<strong>✓ Envio COMPLETADO</strong><br>Orden cerrada por Aduana'
                + ((" · " + adu_fecha) if adu_fecha else "")
                + ((" · " + adu_user) if adu_user else "")
                + "</div>"
            )
        elif can_edit_checklist_aduana():
        ...
        cerrada_ord = orden_cerrada_por_aduana(o)
        if cerrada_ord or (adu_ent_ok and adu_pasos_ok):
            estado_row = '<span class="badge badge-ok">Envio a maquila COMPLETADO</span>'
        elif completo:
            estado_row = '<span class="badge badge-ok">Listo entrega</span>'
        else:
            estado_row = '<span class="badge badge-pend">Pendiente</span>'
        agenda_badge = ""
        if f_sal_raw:
            try:
