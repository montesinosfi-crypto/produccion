...
# -*- coding: utf-8 -*-
"""
Sistema de Producción - Versión completa
Programa → Trazo → Corte (orden + piezas) → Habilitado → Asignación Maquila → Calendario
También: Pedidos especiales sin programa
"""
    ...
    if USE_POSTGRES:
        cur.execute("""
            ...
            CREATE TABLE IF NOT EXISTS ordenes (
                ...
                tipo TEXT DEFAULT 'Programa',
                modelo TEXT, color TEXT, tallas_json TEXT DEFAULT '{}',
                piezas_programadas INT DEFAULT 0,
                piezas_cortadas INT DEFAULT 0,
...
.nav a{color:rgba(255,255,255,.9);text-decoration:none;margin-left:.9rem;font-size:.85rem}
.container{max-width:1050px;margin:1.3rem auto;padding:0 1rem}
.card{background:#fff;border-radius:12px;padding:1.25rem;margin-bottom:1.1rem;box-shadow:0 1px 6px rgba(0,0,0,.07);border:1px solid #e2e8f0}
h1{font-size:1.3rem;color:#0f2744;margin-bottom:.35rem}
h2{font-size:1.05rem;color:#0f2744;margin-bottom:.65rem}
.btn{display:inline-block;padding:.45rem .95rem;border-radius:8px;border:none;font-weight:600;cursor:pointer;text-decoration:none;font-size:.85rem}
...
input,select,textarea{width:100%;padding:.48rem .7rem;border:1px solid #cbd5e1;border-radius:8px;margin-bottom:.8rem;font-size:.88rem}
.form-row{display:flex;gap:.85rem;flex-wrap:wrap}.form-row>div{flex:1;min-width:120px}
.tallas{display:flex;flex-wrap:wrap;gap:.3rem;margin-bottom:.6rem}
.tallas div{text-align:center}.tallas input{width:55px;margin:0}.tallas label{font-size:.68rem}
...
def login():
    ...
    body = f'''<div style="max-width:380px;margin:3rem auto" class="card">
        ...
        <div style="margin-top:1rem;font-size:.75rem;color:#64748b;background:#f8fafc;padding:.7rem;border-radius:8px">
        admin/admin123 · programa/programa123 · trazo/trazo123 · corte/corte123 · gerente/gerente123 · habilitado/habilitado123</div></div>'''
...
def index():
    ...
    nprog = cur.fetchone()["c"] if isinstance(cur.fetchone() if False else (cur.execute("SELECT COUNT(*) as c FROM programas") or cur.fetchone()), dict) else 0
    cur.execute("SELECT COUNT(*) as c FROM programas")
    nprog = cur.fetchone()
    nprog = nprog["c"] if isinstance(nprog, dict) else nprog[0]
    cur.execute("SELECT COUNT(*) as c FROM ordenes")
    nord = cur.fetchone()
    nord = nord["c"] if isinstance(nord, dict) else nord[0]
    cur.execute("SELECT * FROM ordenes ORDER BY id DESC LIMIT 10")
    ...
    body = f'''<div class="card"><h1>Panel de Producción</h1>
        ...
        <div style="margin-top:.8rem;display:flex;gap:.6rem;flex-wrap:wrap">
            ...
            <a href="/agenda" class="btn btn-outline">📅 Agenda Maquila</a>
        </div></div>
        <div class="card"><h2>Órdenes recientes</h2>
        {"<table><thead><tr><th>Orden</th><th>Modelo/Color</th><th>Prog.</th><th>Cortadas</th><th>Estado</th><th></th></tr></thead><tbody>"+rows+"</tbody></table>" if rows else "<p style='color:#64748b'>Sin órdenes aún.</p>"}</div>'''
...
def programas():
    ...
    rows = "".join(f'''<tr><td><strong>{p["numero_programa"]}</strong></td>
        ...
        <td><a href="/programa/{p["id"]}" class="btn btn-outline btn-sm">Ver</a></td></tr>''' for p in progs)
    body = f'''<div class="card"><div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:.6rem">
...
def programa_nuevo():
    ...
    <div><button type="submit" class="btn btn-primary">Crear</button>
    <a href="/programas" class="btn btn-outline">Cancelar</a></div></form></div>
    <script>
...
def ver_programa(prog_id):
    ...
    body = f'''<div class="card"><div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:.6rem">
        ...
        <a href="/programas" class="btn btn-outline">← Volver</a></div></div>
        <div class="card"><h2>Puntos</h2>
        <table><thead><tr><th>Punto</th><th>Modelo</th><th>Color</th><th>Tallas / Prog.</th><th>Trazo</th><th>Orden</th></tr></thead>
...
# ========== CORTE DESDE PUNTO (crea orden completa) ==========
...
def corte_desde_punto(punto_id):
    ...
    if request.method == "POST":
        ...
        user = session.get("nombre") or session.get("usuario")
        if not num:
            return redirect(url_for("corte_desde_punto", punto_id=punto_id))
        conn = get_db(); cur = conn.cursor()
        try:
            if USE_POSTGRES:
    ...
    body = f'''<div class="card"><h1>Asignar Orden (Corte)</h1>
        ...
        <form method="POST" style="margin-top:1rem">
            <label>Número de Orden * (cualquier número)</label>
            <input name="numero_orden" required placeholder="OP-2026-050">
            <div class="form-row">
                <div><label>Piezas Programadas</label>
                    <input type="number" name="piezas_programadas" value="{prog_piezas}" min="0"></div>
                <div><label>Piezas Cortadas</label>
                    <input type="number" name="piezas_cortadas" value="0" min="0"></div>
            </div>
            <div class="form-row">
...
def orden_especial():
    if request.method == "POST":
        ...
        user = session.get("nombre") or session.get("usuario")
        if not num:
            return redirect(url_for("orden_especial"))
        conn = get_db(); cur = conn.cursor()
        try:
            if USE_POSTGRES:
    ...
    body = '''<div class="card"><h1>Pedido Especial (sin programa)</h1>
        <p style="color:#64748b;margin-bottom:1rem">Para cortes que no vienen de un programa de corte.</p>
        <form method="POST">
            <label>Número de Orden *</label><input name="numero_orden" required placeholder="OP-ESP-001">
            <div class="form-row">
            ...
            <label>¿Tampografía lista?</label>
            <select name="tampografia_lista"><option value="No">No</option><option value="Si">Sí</option></select>
            <button type="submit" class="btn btn-primary">Crear Orden Especial</button>
            <a href="/" class="btn btn-outline">Cancelar</a>
        </form></div>'''
    return page("Pedido Especial", body, session)
...
def lista_ordenes():
    ...
    body = f'''<div class="card"><div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:.6rem">
        ...
        <a href="/orden/especial" class="btn btn-warning">+ Especial</a></div></div>
...
def ver_orden(orden_id):
        ...
        if accion == "corte":
            ...
            if USE_POSTGRES:
                cur.execute("""UPDATE ordenes SET piezas_programadas=%s,piezas_cortadas=%s,fecha_inicio_corte=%s,
                    fecha_final_corte=%s,fecha_tampografia=%s,tampografia_lista=%s,estado_actual='Corte' WHERE id=%s""",
                    (p_prog,p_cort,f_ini,f_fin,f_tamp,tamp,orden_id))
            else:
                cur.execute("""UPDATE ordenes SET piezas_programadas=?,piezas_cortadas=?,fecha_inicio_corte=?,
                    fecha_final_corte=?,fecha_tampografia=?,tampografia_lista=?,estado_actual='Corte' WHERE id=?""",
                    (p_prog,p_cort,f_ini,f_fin,f_tamp,tamp,orden_id))
        elif accion == "habilitado":
            est = request.form.get("habilitado_estado","Parcial")
            obs = request.form.get("habilitado_obs","").strip()
            if USE_POSTGRES:
            ...
            if USE_POSTGRES:
                cur.execute("""UPDATE ordenes SET maquilero=%s,fecha_salida_maquila=%s,fecha_entrega_maquila=%s,
                    maquila_usuario=%s,maquila_obs=%s,estado_actual=%s WHERE id=%s""",
                    (maq,f_sal,f_ent,user,obs,estado,orden_id))
            else:
                cur.execute("""UPDATE ordenes SET maquilero=?,fecha_salida_maquila=?,fecha_entrega_maquila=?,
                    maquila_usuario=?,maquila_obs=?,estado_actual=? WHERE id=?""",
                    (maq,f_sal,f_ent,user,obs,estado,orden_id))
        conn.commit(); conn.close()
    ...
    <div class="card"><h2>1. Corte</h2>
        <p>Programadas: <strong>{o.get("piezas_programadas") or 0}</strong> ·
           Cortadas: <strong>{o.get("piezas_cortadas") or 0}</strong>
           {" · <span style='color:#059669'>OK</span>" if dif>=0 else f" · <span style='color:#dc2626'>Faltan {abs(dif)}</span>"}</p>
        ...
        <details style="margin-top:.6rem"><summary style="cursor:pointer;font-weight:600">Editar Corte</summary>
        <form method="POST" style="margin-top:.6rem">
            <input type="hidden" name="accion" value="corte">
            <div class="form-row">
                <div><label>Piezas Programadas</label><input type="number" name="piezas_programadas" value="{o.get("piezas_programadas") or 0}"></div>
                <div><label>Piezas Cortadas</label><input type="number" name="piezas_cortadas" value="{o.get("piezas_cortadas") or 0}"></div>
            </div>
            <div class="form-row">
            ...
            <button type="submit" class="btn btn-primary btn-sm">Guardar Corte</button>
        </form></details>
    </div>
    ...
    <div class="card"><h2>2. Habilitado</h2>
        ...
        <details style="margin-top:.6rem"><summary style="cursor:pointer;font-weight:600">Registrar / Editar</summary>
        <form method="POST" style="margin-top:.6rem">
            <input type="hidden" name="accion" value="habilitado">
            <label>Estado</label>
            <select name="habilitado_estado">
            ...
            <button type="submit" class="btn btn-primary btn-sm">Guardar Habilitado</button>
        </form></details>
    </div>
    </div>
    ...
    <div class="card"><h2>3. Asignación a Maquila</h2>
        ...
        <details style="margin-top:.6rem"><summary style="cursor:pointer;font-weight:600">Registrar / Editar</summary>
        <form method="POST" style="margin-top:.6rem">
            <input type="hidden" name="accion" value="maquila">
            <div class="form-row">
                ...
                <div><label>Fecha Entrega Maquilero</label><input type="date" name="fecha_entrega_maquila" value="{o.get("fecha_entrega_maquila") or ""}"></div>
            </div>
            <label>Observaciones</label><textarea name="maquila_obs" rows="2">{o.get("maquila_obs") or ""}</textarea>
            <button type="submit" class="btn btn-primary btn-sm">Guardar Maquila</button>
        </form></details>
    </div>'''
...
def agenda():
    ...
    for i in range(15):
        ...
        ords = cur.fetchall()
        dias.append({"fecha":f,"fs":fs,"nombre":trad.get(f.strftime("%A"),f.strftime("%A")),
                     "ordenes":ords,"n":len(ords),
                     "piezas":sum((o.get("piezas_cortadas") or o.get("piezas_programadas") or 0) for o in ords)})
    conn.close()
    cards = ""
    for d in dias:
        hoy_badge = ' <span class="badge badge-info">HOY</span>' if d["fs"]==hoy.strftime("%Y-%m-%d") else ""
        rows = "".join(f'''<tr><td><strong>{o["numero_orden"]}</strong></td>
            <td>{o.get("maquilero") or "—"}</td>
            <td>{o.get("piezas_cortadas") or o.get("piezas_programadas") or 0}</td>
            <td><a href="/orden/{o["id"]}" class="btn btn-outline btn-sm">Ver</a></td></tr>''' for o in d["ordenes"])
        cards += f'''<div class="card" style="{"border:2px solid #3b82f6" if d["fs"]==hoy.strftime("%Y-%m-%d") else ""}">
            <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:.4rem;margin-bottom:.5rem">
                <h2 style="margin:0">{d["nombre"]} {d["fecha"].strftime("%d/%m/%Y")}{hoy_badge}</h2>
                <span>{d["n"]} orden(es) · {d["piezas"]} piezas</span>
            </div>
