...
def badge_estado(est):
    m = {"En_Corte":("badge-pend","En corte"),"Listo_Taller":("badge-info","Listo en taller"),
...
def minutos_tendido(fi, hi, ff, hf):
    """Calcula minutos entre inicio y fin de tendido."""
    try:
        if not fi or not ff:
            return None
        hi = (hi or "00:00")[:5]
        hf = (hf or "00:00")[:5]
        a = datetime.strptime(fi + " " + hi, "%Y-%m-%d %H:%M")
        b = datetime.strptime(ff + " " + hf, "%Y-%m-%d %H:%M")
...
def page(title, body, user=None):
    ...
    if user:
        ...
        links_edit = (
            ...
            '<a href="/agenda">Agenda</a><a href="/estadisticas">Estadisticas</a>'
        )
        links_ro = (
            '<a href="/">Inicio</a><a href="/programas">Programas</a>'
...
def lista_ordenes():
    ...
    def opt(lista, sel):
        ...
        return h
    estados = ["En_Corte","Listo_Taller","Enviado_Maquila","Entregado_Parcial","Terminado","Corte","Habilitado","En_Maquila"]
...
def ver_orden(orden_id):
    ...
    if request.method == "POST":
        ...
        if accion == "corte":
            ...
            ff = request.form.get("fecha_fin","").strip()
            hf = request.form.get("hora_fin","").strip()
            bb = int(request.form.get("bloques_base") or 0)
            be = int(request.form.get("bloques_entretela") or 0)
            bc = int(request.form.get("bloques_combinacion") or 0)
            obs = request.form.get("tendido_obs","").strip()
            tid = request.form.get("tendido_id","").strip()
            if USE_POSTGRES:
                if tid and tid.isdigit():
                    cur.execute("""UPDATE tendidos SET persona1=%s,persona2=%s,fecha_inicio=%s,hora_inicio=%s,
                        fecha_fin=%s,hora_fin=%s,bloques_base=%s,bloques_entretela=%s,bloques_combinacion=%s,
                        observaciones=%s WHERE id=%s AND orden_id=%s""",
                        (p1,p2,fi,hi,ff,hf,bb,be,bc,obs,int(tid),orden_id))
                else:
                    cur.execute("""INSERT INTO tendidos (orden_id,persona1,persona2,fecha_inicio,hora_inicio,fecha_fin,hora_fin,
                        bloques_base,bloques_entretela,bloques_combinacion,observaciones,registrado_por)
                        VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""",
                        (orden_id,p1,p2,fi,hi,ff,hf,bb,be,bc,obs,user))
            else:
                if tid and tid.isdigit():
                    cur.execute("""UPDATE tendidos SET persona1=?,persona2=?,fecha_inicio=?,hora_inicio=?,
                        fecha_fin=?,hora_fin=?,bloques_base=?,bloques_entretela=?,bloques_combinacion=?,
                        observaciones=? WHERE id=? AND orden_id=?""",
                        (p1,p2,fi,hi,ff,hf,bb,be,bc,obs,int(tid),orden_id))
                else:
                    cur.execute("""INSERT INTO tendidos (orden_id,persona1,persona2,fecha_inicio,hora_inicio,fecha_fin,hora_fin,
                        bloques_base,bloques_entretela,bloques_combinacion,observaciones,registrado_por)
        ...
        try:
            msgs = {
                ...
                "tendido": "Se registro / actualizo tendido",
            }
    ...
    for t in (tendidos or []):
        ...
        tot_b = (t.get("bloques_base") or 0) + (t.get("bloques_entretela") or 0) + (t.get("bloques_combinacion") or 0)
        prod = ("%.1f bloques/hora" % (tot_b / (mins/60.0))) if mins and mins > 0 and tot_b else "-"
        pareja = (t.get("persona1") or "-") + " + " + (t.get("persona2") or "-")
        tendido_rows += (
            "<tr><td>" + pareja + "</td>"
            "<td>" + str(t.get("fecha_inicio") or "") + " " + str(t.get("hora_inicio") or "") + "</td>"
            "<td>" + str(t.get("fecha_fin") or "") + " " + str(t.get("hora_fin") or "") + "</td>"
            "<td>" + str(mins if mins is not None else "-") + " min</td>"
            "<td>B:" + str(t.get("bloques_base") or 0) + " E:" + str(t.get("bloques_entretela") or 0) + " C:" + str(t.get("bloques_combinacion") or 0) + "</td>"
            "<td>" + prod + "</td></tr>"
        )
    tendido_table = (
        ('<table><thead><tr><th>Pareja</th><th>Inicio</th><th>Fin</th><th>Duracion</th><th>Bloques</th><th>Productividad</th></tr></thead><tbody>'
         + tendido_rows + "</tbody></table>") if tendido_rows else '<p style="color:#94a3b8">Sin tendidos registrados.</p>'
    )
    form_tendido = ""
    if can_edit():
        form_tendido = (
            '<details style="margin-top:.6rem" open><summary style="cursor:pointer;font-weight:600">Registrar tendido</summary>'
            '<form method="POST" style="margin-top:.55rem"><input type="hidden" name="accion" value="tendido">'
            '<div class="form-row">'
            '<div><label>Persona 1 *</label><input name="persona1" required placeholder="Nombre"></div>'
            '<div><label>Persona 2 *</label><input name="persona2" required placeholder="Nombre"></div></div>'
            '<div class="form-row">'
            ...
            '<div><label>Hora fin</label><input type="time" name="hora_fin" required></div></div>'
            '<div class="form-row">'
            '<div><label>Bloques tela base (3-6)</label><input type="number" name="bloques_base" min="0" max="10" value="4"></div>'
            '<div><label>Bloques entretela (1-2)</label><input type="number" name="bloques_entretela" min="0" max="5" value="1"></div>'
            '<div><label>Bloques combinacion (0-2)</label><input type="number" name="bloques_combinacion" min="0" max="5" value="0"></div></div>'
            '<label>Observaciones</label><textarea name="tendido_obs" rows="2"></textarea>'
            '<button type="submit" class="btn btn-primary btn-sm">Guardar tendido</button></form></details>'
        )
    seccion_tendido = (
        '<div class="card"><h2>1b. Tendido (antes de cortar)</h2>'
        '<p style="color:#64748b;font-size:.85rem;margin-bottom:.5rem">Opcional. Sirve para medir productividad de parejas. Si no lo usas, el resto del sistema sigue igual.</p>'
    ...
    <div class="card"><h2>3. Maquila (envio y entrega)</h2>
        ...
        <details style="margin-top:.55rem" open><summary style="cursor:pointer;font-weight:600">Gestionar maquila</summary>
        <form method="POST" style="margin-top:.55rem"><input type="hidden" name="accion" value="maquila">
            <div class="form-row">
                ...
                <div><label>Piezas enviadas</label><input type="number" name="piezas_enviadas" value="{o.get("piezas_enviadas") or o.get("piezas_cortadas") or 0}"></div>
            </div>
            <div class="form-row">
                <div><label>Fecha Entrega</label><input type="date" name="fecha_entrega_maquila" value="{o.get("fecha_entrega_maquila") or ""}"></div>
                <div><label>Piezas entregadas ahora</label><input type="number" name="piezas_entregadas" value="0"></div>
            </div>
            ...
            <div style="display:flex;gap:.5rem;flex-wrap:wrap;margin-top:.4rem">
                <button type="submit" name="marcar_enviado" value="1" class="btn btn-warning btn-sm"
                    onclick="return confirm('Marcar como ENVIADO a maquila?')">Marcar como enviado a maquila</button>
...
def estadisticas():
    ...
    try:
        cur.execute("SELECT * FROM tendidos ORDER BY id DESC LIMIT 300")
        ...
        for t in all_t:
            ...
            mins = minutos_tendido(t.get("fecha_inicio"), t.get("hora_inicio"), t.get("fecha_fin"), t.get("hora_fin"))
            tot = (t.get("bloques_base") or 0) + (t.get("bloques_entretela") or 0) + (t.get("bloques_combinacion") or 0)
            if mins is None or mins <= 0:
                continue
            key = " + ".join(sorted([p for p in [p1, p2] if p]))
            if key:
                pairs.setdefault(key, {"mins": 0, "bloques": 0, "n": 0})
                pairs[key]["mins"] += mins
                pairs[key]["bloques"] += tot
                pairs[key]["n"] += 1
            for p in [p1, p2]:
                if not p:
                    continue
                persons.setdefault(p, {"mins": 0, "bloques": 0, "n": 0})
                persons[p]["mins"] += mins
                persons[p]["bloques"] += tot
                persons[p]["n"] += 1
        tendido_prod = sorted(pairs.items(), key=lambda x: x[1]["bloques"], reverse=True)
        tendido_pers = sorted(persons.items(), key=lambda x: x[1]["bloques"], reverse=True)
    except Exception as e:
    ...
    def rows_act(lista):
        if not lista: return "<tr><td colspan=5>Sin datos</td></tr>"
        r = ""
        for x in lista:
...
""" % (
    ...
    body = (
        ...
        + '<div class="card"><h2>Productividad de tendido — parejas</h2>'
        + '<table><thead><tr><th>Pareja</th><th>Tendidos</th><th>Minutos</th><th>Bloques</th><th>Bloques/hora</th></tr></thead><tbody>'
        + ("".join("<tr><td><strong>%s</strong></td><td>%s</td><td>%s</td><td>%s</td><td>%.1f</td></tr>" % (
            k, v["n"], v["mins"], v["bloques"], (v["bloques"] / (v["mins"] / 60.0) if v["mins"] else 0)
          ) for k, v in tendido_prod) if tendido_prod else "<tr><td colspan=5>Sin datos de tendido</td></tr>")
        + "</tbody></table></div>"
        + '<div class="card"><h2>Productividad de tendido — por persona</h2>'
        + '<table><thead><tr><th>Persona</th><th>Tendidos</th><th>Minutos</th><th>Bloques</th><th>Bloques/hora</th></tr></thead><tbody>'
        + ("".join("<tr><td><strong>%s</strong></td><td>%s</td><td>%s</td><td>%s</td><td>%.1f</td></tr>" % (
            k, v["n"], v["mins"], v["bloques"], (v["bloques"] / (v["mins"] / 60.0) if v["mins"] else 0)
          ) for k, v in tendido_pers) if tendido_pers else "<tr><td colspan=5>Sin datos</td></tr>")
        + "</tbody></table></div>"
        + chart_js
    )
