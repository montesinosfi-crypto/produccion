...
app = Flask(__name__)
APP_VERSION = "2026-09-24-v5-aduana-prod"
...
def productividad():
    ...
    try:
        cur.execute(
            "SELECT t.*, o.numero_orden, o.modelo, o.color, o.tipo, o.piezas_cortadas, o.fecha_final_corte, "
    ...
    for t in tendidos:
        ...
        for p in pair:
            ...
            # piezas equivalentes completas por persona (no se dividen entre pareja) + factores
            tipo_o = t.get("tipo") or "Programa"
            pzas_eq = piezas_productividad(pzas, modelo, tipo_o)
            if pzas_eq > 0 and oid and oid not in st["ordenes_corte"]:
                st["piezas_corte"] += pzas_eq
                st["ordenes_corte"].add(oid)
            st["detalle"].append({
                "dia": dia, "orden": num, "modelo": modelo,
                "mins": mins or 0, "mins_cal": mins_cal or 0,
                "metros": metros, "bloques": n_bl,
                "piezas": pzas_eq if (oid and oid) else 0,
                "piezas_raw": pzas,
                "tipo": tipo_o,
                "factor": (factor_modelo(modelo) * factor_tipo_orden(tipo_o)),
    ...
    if persona_q and persona_q in personas:
        ...
        for d in sorted(st["detalle"], key=lambda x: (x["dia"], x["orden"]), reverse=True):
            ...
            fac = d.get("factor") or 1
            raw = d.get("piezas_raw")
            pnote = ""
            if raw and fac and fac != 1:
                pnote = " <span style='color:#64748b;font-size:.72rem'>(%s × %.1f)</span>" % (int(raw), fac)
            drows += (
                "<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%.1f m</td>"
                "<td><strong>%s min</strong>%s</td><td>%.0f%s</td></tr>"
            ) % (
                d["dia"], d["orden"], d.get("modelo") or "—", d["bloques"], d["metros"],
                d["mins"], nota, d["piezas"], pnote
            )
        detalle_html = (
            ...
            '<table><thead><tr><th>Fecha</th><th>Orden</th><th>Modelo</th><th>Bloques</th><th>Metros</th><th>Minutos laborales</th><th>Piezas equiv.*</th></tr></thead>'
            '<tbody>%s</tbody></table>'
            '<p style="font-size:.8rem;color:#64748b;margin-top:.5rem">Minutos = solo jornada (Lun 8-14/15-18, Mar-Vie 8-14/15-17:30). '
            'No cuenta noche ni comida. Piezas equivalentes = cortadas × factor modelo × factor tipo (especial×2). No se dividen entre la pareja: cada uno recibe el total del equipo. Pijama×2, bata×1.5, gorro/bolsa×0.5, resto×1.</p>'
    ...
    body = (
        ...
        'Horas = jornada laboral. Piezas = total del equipo (no se parten entre pareja) × factores: especial×2, pijama×2, bata×1.5, gorro/bolsa×0.5.</p>'
        '<table><thead><tr>'
