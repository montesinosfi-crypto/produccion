...
def ver_orden(orden_id):
        ...
        if accion == "corte":
            ...
            if USE_POSTGRES:
                cur.execute("""INSERT INTO tendidos (orden_id,persona1,persona2,fecha_inicio,hora_inicio,fecha_fin,hora_fin,
                    bloques_base,bloques_entretela,bloques_combinacion,observaciones,registrado_por,bloques_json)
    ...
    # --- Tendido HTML (tiempo + pareja + bloques) ---
    tendido_rows = ""
    for t in (tendidos or []):
        ...
        if bl:
            ...
            for b in bl:
                ...
                bl_txt += " · " + str(b.get("lienzos") or "") + " lienzos · largo " + str(b.get("largo") or "") + "</li>"
            bl_txt += "</ul>"
        else:
        ...
        tendido_rows += (
            ...
            "<td>" + bl_txt + "</td>"
            "<td>" + str(t.get("observaciones") or "") + "</td></tr>"
        )
    tendido_table = (
        ('<table><thead><tr><th>Pareja</th><th>Inicio</th><th>Fin</th><th>Duracion</th><th>Bloques</th><th>Obs.</th></tr></thead><tbody>'
    ...
    if can_edit():
        form_tendido = (
            ...
            '<h3 style="font-size:.95rem;margin:0.8rem 0 0.4rem">Bloques del tendido</h3>'
            ...
            + '<div><label>Tela</label><input name="bloque_tela[]" required placeholder="Ej. Base, Entretela, Combinacion"></div>'
            + '<div><label>Color</label><input name="bloque_color[]" placeholder="Color"></div>'
            + '<div><label>Cantidad de lienzos</label><input type="number" name="bloque_lienzos[]" min="0" step="1" placeholder="0"></div>'
            + '<div><label>Largo del bloque</label><input name="bloque_largo[]" placeholder="Ej. 2.5 m"></div>'
            + '</div><button type="button" class="btn btn-outline btn-sm" onclick="this.parentNode.remove()">Quitar bloque</button>`;'
            'c.appendChild(d);}'
            'if(document.getElementById("bloquesTendido")&&!document.getElementById("bloquesTendido").children.length)addBloqueTendido();'
            '</script>'
        )
    seccion_tendido = (
        '<div class="card"><h2>1b. Tendido (antes de cortar)</h2>'
        '<p style="color:#64748b;font-size:.85rem;margin-bottom:.5rem">Pareja, tiempo y bloques (tela, color, lienzos, largo). Opcional.</p>'
...
""" % (
    ...
    body = (
        ...
        + '<table><thead><tr><th>Pareja</th><th>Tendidos</th><th>Minutos totales</th><th>Horas</th></tr></thead><tbody>'
        + ("".join("<tr><td><strong>%s</strong></td><td>%s</td><td>%s</td><td>%.1f</td></tr>" % (
            k, v["n"], v["mins"], (v["mins"] / 60.0 if v["mins"] else 0)
        ...
        + '<table><thead><tr><th>Persona</th><th>Tendidos</th><th>Minutos totales</th><th>Horas</th></tr></thead><tbody>'
        + ("".join("<tr><td><strong>%s</strong></td><td>%s</td><td>%s</td><td>%.1f</td></tr>" % (
            k, v["n"], v["mins"], (v["mins"] / 60.0 if v["mins"] else 0)
