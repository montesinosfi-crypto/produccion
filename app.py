...
@login_required
def checklist():
    """Checklist de procesos por orden: tendido, corte, habilitado, tampografia, salida maquila."""
    filtro = (request.args.get("filtro") or "pendientes").strip()  # pendientes | completas | todas
    ...
    try:
        ...
        ordenes = []
    # tendidos by orden_id for dates
    tend_map = {}
    try:
    ...
    for o in ordenes:
        ...
        if q:
            blob = (" ".join([str(num), str(modelo), str(color), str(maq)])).lower()
            if q.lower() not in blob:
                continue

        # Tendido: al menos un registro; completo si alguno tiene fin
        ts = tend_map.get(oid) or []
        tiene_tendido = len(ts) > 0
        tendido_completo = any((t.get("fecha_fin") and t.get("hora_fin")) for t in ts)
        # fecha tendido: primer inicio
        f_tend = "—"
        if ts:
            t0 = ts[0]
            f_tend = fmt_fecha(
                (str(t0.get("fecha_inicio") or "")[:10] + " " + str(t0.get("hora_inicio") or "")).strip()
            )
            if tendido_completo:
                # mostrar tambien fin del ultimo completo
                for t in reversed(ts):
                    if t.get("fecha_fin") and t.get("hora_fin"):
                        f_tend = fmt_fecha(
                            str(t.get("fecha_inicio") or "")[:10] + " " + str(t.get("hora_inicio") or ""),
                        ) + " → " + fmt_fecha(
                            str(t.get("fecha_fin") or "")[:10] + " " + str(t.get("hora_fin") or "")
                        )
                        break

        # Corte: tiene piezas cortadas o fecha final
        ...
        f_hab = "—"  # no hay fecha dedicada; mostrar estado
        if hab:
            f_hab = hab + ((" · " + str(o.get("habilitado_usuario") or "")) if o.get("habilitado_usuario") else "")
        ...
        # Para checklist de "listo para enviar completo": salida programada cuenta como check de datos
        prog_ok = bool(maq != "—" and f_sal)
        f_salida = fmt_fecha(f_sal)
        if maq != "—":
            f_salida = (f_salida if f_salida != "—" else "Programado") + " · " + maq

        # Completo para entrega: tendido (si hubo o no obligatorio), corte, habilitado, tampografia, y datos de maquila
        # Criterio entrega completa del corte:
        checks = [corte_ok, hab_ok, tamp_ok, prog_ok]
        # tendido: si hay tendidos, debe estar completo; si no hay, no bloquea (pedidos especiales)
        if tiene_tendido:
            checks.insert(0, tendido_completo)
        else:
            checks.insert(0, True)  # sin tendido registrado no bloquea checklist visual de "completo envio"
        completo = all(checks)
        n_all += 1
        if completo:
            n_ok += 1
        ...
        rows += (
            "<tr>"
            '<td><strong><a href="/orden/%s">%s</a></strong></td>'
            ...
            ok_icon(tendido_completo if tiene_tendido else False) if tiene_tendido else '<span style="color:#94a3b8" title="Sin tendido">—</span>',
    ...
    for key, label in [("pendientes", "Pendientes"), ("completas", "Listas entrega"), ("todas", "Todas")]:
        ...
        tabs += '<a class="btn %s btn-sm" href="/checklist?filtro=%s&q=%s">%s</a> ' % (cls, key, q, label)

    body = (
        '<div class="card"><h1>Checklist de procesos</h1>'
        '<p style="color:#64748b">Confirma que el corte va completo antes de entrega / salida a maquila: '
        'Tendido · Corte · Habilitado · Tampografia · Salida (maquilero y fecha).</p>'
        '<div style="margin:.6rem 0;display:flex;flex-wrap:wrap;gap:.5rem;align-items:center">' + tabs +
        '<form method="GET" style="display:inline-flex;gap:.4rem;align-items:center">'
        '<input type="hidden" name="filtro" value="' + filtro + '">'
        '<input name="q" value="' + q + '" placeholder="Buscar orden, modelo, maquilero" style="margin:0;min-width:180px">'
        '<button type="submit" class="btn btn-outline btn-sm">Buscar</button></form></div>'
        '<p style="font-size:.9rem">Pendientes: <strong>%s</strong> · Listas: <strong>%s</strong> · Total vistas: <strong>%s</strong></p></div>'
        % (n_pend, n_ok, n_all)
        +
        '<div class="card" style="overflow-x:auto">'
        '<table style="font-size:.8rem"><thead><tr>'
        '<th>Orden</th><th>Modelo / Color</th><th>Maquilero</th>'
        '<th>Tendido</th><th>Corte</th><th>Habilitado</th><th>Tampografia</th><th>Salida maquila</th><th>Estado</th>'
        '</tr></thead><tbody>'
        + (rows or '<tr><td colspan="9">No hay ordenes con este filtro</td></tr>')
        + '</tbody></table>'
        '<p style="font-size:.78rem;color:#64748b;margin-top:.8rem">'
        '✓ = proceso con datos · ○ = pendiente · '
        '<strong>Listo entrega</strong> = corte + habilitado + tampografia + maquilero/fecha salida. '
        'Tendido: si hay registros, debe tener hora de fin.</p></div>'
    )
