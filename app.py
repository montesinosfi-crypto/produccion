...
def page(title, body, user=None):
    ...
    if user:
        if can_edit():
            ...
            <a href="/agenda">Agenda</a><a href="/estadisticas">Estadisticas</a></div>
...
def estadisticas():
    ...
    row = cur.fetchone()
    corte_sem_p = row["pzas"] if isinstance(row, dict) else row[0]
    corte_sem_n = row["n"] if isinstance(row, dict) else row[1]
    cur.execute(q, (mini_s, mfin_s))
    row = cur.fetchone()
    corte_mes_p = row["pzas"] if isinstance(row, dict) else row[0]
    corte_mes_n = row["n"] if isinstance(row, dict) else row[1]
    ...
    def rows_act(lista):
        ...
        r = ""
        for x in lista:
            env = x["enviadas"] if isinstance(x, dict) else x[2]
            ent = x["entregadas"] if isinstance(x, dict) else x[3]
            pend = (env or 0) - (ent or 0)
            nom = x["maquilero"] if isinstance(x, dict) else x[0]
            n = x["ordenes"] if isinstance(x, dict) else x[1]
            r += "<tr><td><strong>%s</strong></td><td>%s</td><td>%s</td><td>%s</td><td style=\"color:#d97706\">%s</td></tr>" % (nom, n, env, ent, pend)
        return r
    def rows_ent(lista):
        ...
        for x in lista:
            nom = x["maquilero"] if isinstance(x, dict) else x[0]
            n = x["ordenes"] if isinstance(x, dict) else x[1]
            p = x["pzas"] if isinstance(x, dict) else x[2]
            r += "<tr><td><strong>%s</strong></td><td>%s</td><td>%s</td></tr>" % (nom, n, p)
        return r
    body = (
        '<div class="card"><h1>Estadisticas</h1>'
        '<p style="color:#64748b">Semana laboral: ' + lun.strftime("%d/%m") + " - " + vie.strftime("%d/%m/%Y") + " (lun-vie) | Mes: " + mini.strftime("%m/%Y") + "</p></div>"
        '<div class="card"><h2>Corte - produccion</h2>'
        '<p><strong>' + str(corte_sem_p) + "</strong> piezas cortadas esta semana (" + str(corte_sem_n) + " ordenes)</p>"
        '<p><strong>' + str(corte_mes_p) + "</strong> piezas cortadas este mes (" + str(corte_mes_n) + " ordenes)</p></div>"
        '<div class="card"><h2>Maquileros activos (no terminados)</h2>'
        '<p style="color:#64748b;font-size:.85rem">Al entregar el 100% salen de esta lista.</p>'
