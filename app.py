            ...
            if USE_POSTGRES:
                if tid and str(tid).isdigit():
                    cur.execute("""UPDATE tendidos SET persona1=%s,persona2=%s,fecha_inicio=%s,hora_inicio=%s,
                        fecha_fin=%s,hora_fin=%s,observaciones=%s,bloques_json=%s WHERE id=%s AND orden_id=%s""",
                        (p1,p2,fi,hi,ff or None,hf or None,obs,bj,int(tid),orden_id))
                else:
                    cur.execute("""INSERT INTO tendidos (orden_id,persona1,persona2,fecha_inicio,hora_inicio,fecha_fin,hora_fin,
                        ...
                        (orden_id,p1,p2,fi,hi,ff or None,hf or None,obs,user,bj))
            else:
                if tid and str(tid).isdigit():
                    cur.execute("""UPDATE tendidos SET persona1=?,persona2=?,fecha_inicio=?,hora_inicio=?,
                        fecha_fin=?,hora_fin=?,observaciones=?,bloques_json=? WHERE id=? AND orden_id=?""",
                        (p1,p2,fi,hi,ff or None,hf or None,obs,bj,int(tid),orden_id))
                else:
                    cur.execute("""INSERT INTO tendidos (orden_id,persona1,persona2,fecha_inicio,hora_inicio,fecha_fin,hora_fin,
                        bloques_base,bloques_entretela,bloques_combinacion,observaciones,registrado_por,bloques_json)
                        VALUES (?,?,?,?,?,?,?,0,0,0,?,?,?)""",
                        (orden_id,p1,p2,fi,hi,ff or None,hf or None,obs,user,bj))
            # Estado: En_Tendido si hay tendido incompleto; si todos completos y habia En_Tendido, queda Listo o En_Corte
            if not completo:
                if USE_POSTGRES:
                ...
                # si todos los tendidos de la orden estan completos, no forzar En_Tendido
                pass
...
def estadisticas():
    ...
    try:
        ...
        for t in all_t:
            ...
            mins = minutos_tendido(t.get("fecha_inicio"), t.get("hora_inicio"), t.get("fecha_fin"), t.get("hora_fin"))
            try:
                bls = json.loads(t.get("bloques_json") or "[]")
            except Exception:
                bls = []
            tot = len(bls) if bls else ((t.get("bloques_base") or 0) + (t.get("bloques_entretela") or 0) + (t.get("bloques_combinacion") or 0))
            # cuenta siempre; minutos solo si completo
...
""" % (
    ...
    body = (
        ...
        + '<table><thead><tr><th>Pareja</th><th>Tendidos</th><th>Bloques</th><th>Minutos</th><th>Horas</th></tr></thead><tbody>'
        + ("".join("<tr><td><strong>%s</strong></td><td>%s</td><td>%s</td><td>%s</td><td>%.1f</td></tr>" % (
            k, v["n"], v.get("bloques",0), v["mins"], (v["mins"] / 60.0 if v["mins"] else 0)
        ...
        + '<table><thead><tr><th>Persona</th><th>Tendidos</th><th>Bloques</th><th>Minutos</th><th>Horas</th></tr></thead><tbody>'
        + ("".join("<tr><td><strong>%s</strong></td><td>%s</td><td>%s</td><td>%s</td><td>%.1f</td></tr>" % (
            k, v["n"], v.get("bloques",0), v["mins"], (v["mins"] / 60.0 if v["mins"] else 0)
